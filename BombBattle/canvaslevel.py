...
class Level:
    ...
    def __init__(self, canvas, matrix, players, monsters=[]):
        self.canvas = canvas
        self.matrix = matrix
    ...
    def loop(self, time):
        ...
        # self.player.loop(self, time)
        # if(self.player2 != None):
        #     self.player2.loop(self, time)
    ...
    @staticmethod
    def doubleplayer(game, canvas, monsters_lim=[3, 4], boxes_lim=[15, 35], 
            max_bomb = [1,1],
            bomb_blast_radius = [2,2],
            lives = [3,3]):
        # số lượng bom
        monsters_number = random.randrange(monsters_lim[0], monsters_lim[1]+1)#random from 3 to 4
        ...
        players = []
        if(lives[0]>0):
            players += [Player(game, 1, 1,control = Settings().DEFAULT_PLAYER_2_CONTROLS, max_bomb = max_bomb[0] , bomb_blast_radius = bomb_blast_radius[0])]
        if(lives[1]>0):
            players += [Player(game, 11, 1, max_bomb = max_bomb[1] , bomb_blast_radius = bomb_blast_radius[1])]
        # vẽ đồ họa
        for x in range(0, 13):
            for y in range(0, 13): 
    ...
    def singleplayer(game, canvas, monsters_lim=[3, 4], boxes_lim=[15, 35], max_bomb = [1], bomb_blast_radius = [2]):
        ...
        matrix = [[None]*13 for _ in range(13)]
        players = [Player(game, 1, 1, max_bomb = max_bomb[0] , bomb_blast_radius = bomb_blast_radius[0])]
        # vẽ đồ họa
        for x in range(0, 13):
            for y in range(0, 13): 
                if x == 0 or x == 12 or y == 0 or y == 12:
